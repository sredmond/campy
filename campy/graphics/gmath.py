...
Students often represent 2D vectors by their components, so we also provide
conversion to vector magnitude and vector angle.

The mathematical constants ``PI`` and ``E`` are also exported so that students
don't have to import a separate math module for those constants.
"""
import math
